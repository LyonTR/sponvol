...
def list(table, limit=10, cursor=None):
    if table == "Spolunteers"
        ...
        query = (Interest.query
                 ...
                 .limit(limit)
                 .offset(cursor))
        interest = builtin_list(map(from_sql, query.all()))
        next_page = cursor + limit if len(interest) == limit else None
        return (interest, next_page)
# [END list]
...
def create(table, data):
    if table == "Spolunteers":
        content = Volunteer(**data)
    elif table == "Interest":
        content = Interest(**data)
    db.session.add(content)
    db.session.commit()
    return from_sql(content)
# [END create]
...
def update(table, data, id):
    if table == "Spolunteers":
        content = Volunteer.query.get(id)
    elif table == "Interest":
        content = Interest.query.get(id)
...
def delete(table, id):
    if table == "Spolunteers":
        ...
        Interest.query.filter_by(id=id).delete()
    db.session.commit()
